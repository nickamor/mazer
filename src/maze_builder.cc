...
Cell *
MazeBuilder::CellAt(CellPos pos)
{
    return CellAt(pos.x, pos.y);
}

Cell *
MazeBuilder::CellAt(int x, int y)
{
    if (!ValidCell(x, y))
    {
...
void
MazeBuilder::AddLink(CellPos lhs, CellPos rhs)
{
    auto left = CellAt(lhs);
    auto right = CellAt(rhs);
    AddLink(left, right);
}

void
MazeBuilder::AddLink(Cell *lhs, Cell *rhs)
{
