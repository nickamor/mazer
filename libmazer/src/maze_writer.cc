...
MazeWriter::Write(std::shared_ptr<Maze> maze)
{
    ...
    for (auto &edge : edges)
    {
        stream.write((const char *)(&edge.src.x), sizeof edge.src.x);
        stream.write((const char *)(&edge.src.y), sizeof edge.src.y);
        stream.write((const char *)(&edge.dst.x), sizeof edge.dst.x);
        stream.write((const char *)(&edge.dst.y), sizeof edge.dst.y);
    }
